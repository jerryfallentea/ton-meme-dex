...
export default function TradeModal({ onClose }) {
  ...
  const [usdAmount, setUsdAmount]     = useState('');   // buy: user types USD to spend
  const [tokenAmount, setTokenAmount] = useState('');   // sell: user types token quantity
  ...
  // Fetch portfolio when token + wallet available
  useEffect(() => {
  ...
  const sellTokens  = tradeTab === 'sell'
    ? (holding * sellPct) / 100
    ...
    try {
      const r = await fetch(`${API}/api/orders`, {
        ...
        body: JSON.stringify({
          ...
          target_price: parseFloat(tpPrice),
          amount: tpAmount,
          percentage: tpPct,
        }),
      });
          ...
          <>
            <div style={{ padding: '16px 16px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                <div style={{ fontWeight: 700, fontSize: '16px' }}>⚡ Quick Trade</div>
          ...
          <>
            ...
            <div style={{ padding: '16px', overflowY: 'auto', flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                {['buy', 'sell'].map((t) => (
                  <button key={t} onClick={() => { setTradeTab(t); setTradeError(''); }}
                    style={{
                      flex: 1, padding: '10px', borderRadius: '8px', fontWeight: 700, fontSize: '14px',
                      background: tradeTab === t ? (t === 'buy' ? 'var(--green)' : 'var(--red)') : 'transparent',
                      color: tradeTab === t ? '#fff' : 'var(--text-secondary)',
                      border: `1px solid ${tradeTab === t ? (t === 'buy' ? 'var(--green)' : 'var(--red)') : 'var(--border)'}`,
                    ...
                    {USD_PRESETS.map((p) => (
                      <button key={p} onClick={() => setUsdAmount(String(p))}
                        style={{
                          ...
                          background: usdAmount === String(p) ? 'var(--green)' : 'var(--bg-hover)',
                          color: usdAmount === String(p) ? '#fff' : 'var(--text-secondary)',
                          border: `1px solid ${usdAmount === String(p) ? 'var(--green)' : 'var(--border)'}`,
                        }}
                      >${p}</button>
                    ))}
                  </div>
                ...
                <>
                  ...
                  {holding > 0 && (
                    <>
                      ...
                      <input type="range" min="1" max="100" value={sellPct}
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginBottom: '14px' }}>
                        {[25, 50, 75].map((p) => (
                          <button key={p} onClick={() => setSellPct(p)}
                            style={{
                              flex: 1, padding: '5px', borderRadius: '6px', fontSize: '12px', fontWeight: 600,
                              background: sellPct === p ? 'var(--red)' : 'var(--bg-hover)',
                              color: sellPct === p ? '#fff' : 'var(--text-secondary)',
                              border: `1px solid ${sellPct === p ? 'var(--red)' : 'var(--border)'}`,
                            }}
                          >{p}%</button>
                        ))}
              ...
              <button onClick={handleTrade} disabled={submitting}
                style={{
                  ...
                  background: connected
                    ? (tradeTab === 'buy' ? 'var(--green)' : 'var(--red)')
                    : 'var(--accent)',
              ...
              >
                {submitting ? 'Processing…'
                  : connected
                    ? `${tradeTab === 'buy' ? 'Buy' : 'Sell'} ${selectedToken.symbol}`
          ...
          <>
            <div style={{ padding: '16px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ fontWeight: 700, fontSize: '16px' }}>🎯 Set Take-Profit</div>
                <button onClick={onClose} style={{ background: 'none', color: 'var(--text-secondary)', fontSize: '22px', lineHeight: 1 }}>×</button>
              </div>
            </div>
            ...
            <div style={{ padding: '16px', overflowY: 'auto', flex: 1 }}>
              {/* Success badge */}
              <div style={{ padding: '12px', background: 'rgba(0,208,132,0.08)', border: '1px solid rgba(0,208,132,0.25)', borderRadius: '10px', marginBottom: '16px', fontSize: '13px' }}>
                <div style={{ fontWeight: 700, color: 'var(--green)', marginBottom: '4px' }}>✅ Position Opened!</div>
              ...
              <div style={{ marginBottom: '14px' }}>
                ...
                <input
                  type="number" step="any" value={tpPrice}
                  onChange={(e) => setTpPrice(e.target.value)}
                  placeholder={`e.g. ${(price * 1.5).toExponential(3)}`}
                  style={inputStyle}
                />
              </div>
              ...
              <div style={{ marginBottom: '14px' }}>
                ...
                <input type="range" min="1" max="100" value={tpPct}
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  {TP_PRESETS.map((p) => (
                    <button key={p} onClick={() => setTpPct(p)}
                      style={{
                        flex: 1, padding: '5px', borderRadius: '6px', fontSize: '12px', fontWeight: 600,
                        background: tpPct === p ? 'var(--accent)' : 'var(--bg-hover)',
                        color: tpPct === p ? '#fff' : 'var(--text-secondary)',
                        border: `1px solid ${tpPct === p ? 'var(--accent)' : 'var(--border)'}`,
                      }}
                    >{p}%</button>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                >
                  {tpLoading ? 'Saving…' : '🎯 Set Take-Profit'}
                </button>
